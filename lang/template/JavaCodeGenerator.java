...
public class JavaCodeGenerator {
    ...
    public JavaCodeGenerator(String fileName, String filePath, Node rootNode, SemanticVisitor semanticVisitor) {
        this.fileName = fileName;
        this.filePath = filePath;
        this.rootNode = rootNode;
        this.semanticVisitor = semanticVisitor;
        this.env = semanticVisitor.getEnv();
    }

    public void generateJavaCode(String[] args) {
        ...
        System.out.println("Traduzindo o Arquivo: \"" + filePath + " para Java\"\n");
        ...
        if (args.length > 2) {
            if (args.length == 3) {
                if (args[2].equals("-genFile")) {
                    jv = new JavaVisitor(fileName, env, semanticVisitor.getDatas());
                    rootNode.accept(jv);
                    String generatedFilePath = getPathFile(filePath) + fileName + ".java";
                ...
                if (args[2].equals("-genFile")) {
                    jv = new JavaVisitor(fileName, env, semanticVisitor.getDatas());
                    rootNode.accept(jv);
                    String generatedFilePath = getPathFile(args[3]) + fileName + ".java";
                    System.out.println("Arquivo gerado: \"" + generatedFilePath + "\"\n");
                    writeFile(generatedFilePath, jv.getTemplate());
                } else {
                    System.out.println("Parâmetro '" + args[2] + "' incorreto");
