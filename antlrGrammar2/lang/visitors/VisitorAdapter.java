...
public class VisitorAdapter extends LangBaseVisitor<Node> {
    ...
    public Node visitDataDeclaration(DataDeclarationContext ctx) {
        ...
        List<Decl> decls = new ArrayList<Decl>(); // Declarações
    ...
    public Node visitFunction(FunctionContext ctx) {
        ...
        Func function = new Func(line, column, ctx.getChild(0).getText()); // Nome da função
    ...
    public Node visitFunctionCall(FunctionCallContext ctx) {
        // ----- Regra
        // cmd: ID OPEN_PARENT exps? CLOSE_PARENT (LESS_THAN lvalue (COMMA lvalue)*
        // GREATER_THAN)? SEMI # FunctionCall
    ...
    public Node visitCharLitteral(CharLitteralContext ctx) {
        ...
        // return new CharLitteral(ctx.getStart().getLine(),
        // ctx.getStart().getCharPositionInLine(),
    ...
    public Node visitTypeInstanciate(TypeInstanciateContext ctx) {
        ...
        if (ctx.type().accept(this) instanceof NameType) {
            ...
            if (ctx.exp() != null) {
                ...
                // Type type = (Type) ctx.type().accept(this);
                // System.out.println("Instancia um TypeInstanciate -- VisitorAdapter(ARRAY
                // COMUM) -- 548 -- " + type.toString() + " -- " + exp.toString());
    ...
    public Node visitArrayAccess(ArrayAccessContext ctx) {
        ...
        // System.out.println("582 - ARRAY Access --- "+ ctx.lvalue().getText() + " ---
        // " + ctx.exp().getText());
    ...
    // return new Identifier(ctx.getStart().getLine(),
    // ctx.getStart().getCharPositionInLine(),
    // ctx.getChild(0).getText());
    // }
    ...
    public Node visitDataAccess(DataAccessContext ctx) {
        ...
        // System.out.println("608 - DATA Access --- "+ ctx.ID().getText() + " --- " +
        // ctx.lvalue().getText() + " --- " + ctx.lvalue().getText());
