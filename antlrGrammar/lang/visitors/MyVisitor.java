...
public class MyVisitor extends LangBaseVisitor<Node> {
    ...
    public Node visitFunName(FunNameContext ctx) {
        ...
        for (int i = 0; i < (ctx.type().size()) && this.shouldVisitNextChild(ctx, this.defaultResult()); i++) {
            ...
            function.addTypes((Type) this.aggregateResult(this.defaultResult(), childTree.accept(this)));
        }

        for (int i = 0; i < (ctx.cmd().size()) && this.shouldVisitNextChild(ctx, this.defaultResult()); i++) {
            ParseTree childTree = ctx.cmd(i);
            function.addCmd((Cmd) this.aggregateResult(this.defaultResult(), childTree.accept(this)));
        }
    ...
    public Node visitLvalueCmd(LvalueCmdContext ctx) {
        ...
        // return new LvalueCmd(ctx.start.getLine(), ctx.start.getCharPositionInLine(),
        // lvalue, expr);
    ...
    public Node visitFuncCallCmd(FuncCallCmdContext ctx) {
        ...
        FuncCallCmd fcall = new FuncCallCmd(ctx.getStart().getLine(), ctx.getStart().getCharPositionInLine(),
                ctx.getChild(0).getText());

        if (ctx.exps() != null) {
            FuncArgs exps = (FuncArgs) ctx.exps().accept(this);

            fcall = new FuncCallCmd(ctx.getStart().getLine(), ctx.getStart().getCharPositionInLine(),
    ...
    public Node visitFuncCallRexp(FuncCallRexpContext ctx) {
        ...
        FuncArgs fCallPar = (FuncArgs) ctx.exps().accept(this);
        Expr exp = (Expr) ctx.exp().accept(this);
        return new FuncCall(ctx.getStart().getLine(), ctx.getStart().getCharPositionInLine(), str, fCallPar, exp);
    }
    ...
    public Node visitProgName(ProgNameContext ctx) {
        ...
        Prog program = new Prog(ctx.getStart().getLine(), ctx.getStart().getCharPositionInLine(), new ArrayList<>()); // Linha
                                                                                                                      // e
        // coluna
        ...
        for (int i = 0; i < (ctx.func().size()) && this.shouldVisitNextChild(ctx, this.defaultResult()); i++) {
            ...
            program.addFunc((Func) this.aggregateResult(this.defaultResult(), childTree.accept(this)));
        }
